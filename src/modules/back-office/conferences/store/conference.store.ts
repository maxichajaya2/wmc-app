...
import { ActionsTypes, Conference as Entity, PayloadConference } from "@/models";
import { ConferenceService as ApiService } from "../services/conference.service";
import { useUsersStore } from "@/modules/back-office/users/store/users.store";

type Payload = PayloadConference;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getConferenceSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredConference");
  },

});

export const useConferenceStore = create<State>()(
