...
import { ActionsTypes, Stand as Entity, PayloadStand } from "@/models";
import { StandService as ApiService } from "../services/stand.service";
import { useUsersStore } from "@/modules/back-office/users/store/users.store";

type Payload = PayloadStand;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getStandSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredStand");
  },

});

export const useStandStore = create<State>()(
