...
import ExhibitorDialog from "./components/Dialog/Dialog"
import { useCheckPermission } from "@/utils"
import { ActionRoles, ModulesRoles } from "@/constants"
import { useStandStore } from "../stands/store/stand.store"
import StandAssignmentDialog from "./components/Dialog/AssignStandDialog"

function ExhibitorsManagementPage() {
  ...
  const filtered = useExhibitorStore(state => state.filtered);
  const filterTerm = useExhibitorStore(state => state.filterTerm);

  const findAll = useExhibitorStore(state => state.findAll);
  const findStands = useStandStore(state => state.findAll);
  const setFilterTerm = useExhibitorStore(state => state.setFilterTerm);
    ...
    <main className="grid flex-1 items-start p-2 overflow-auto">
      ...
      <div className="overflow-auto">
        <Card x-chunk="dashboard-06-chunk-0">
          <CardHeader>
            <CardTitle>Exhibidores</CardTitle>
            <CardDescription>
      ...
      {isOpenDialog && <ExhibitorDialog />}
      {isOpenAssignStandDialog && <StandAssignmentDialog />}
    </main>
  )
}
