...
import { ActionsTypes, PressRelease as Entity, PayloadPressRelease } from "@/models";
import { PressReleaseService as ApiService } from "../services/press-releases.service";
import { useUsersStore } from "@/modules/back-office/users/store/users.store";

type Payload = PayloadPressRelease;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getPressReleaseSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    const filtered = data.filter((item) =>
      item.titleEn?.toLowerCase().includes(filterTerm.toLowerCase()) ||
      item.titleEs?.toLowerCase().includes(filterTerm.toLowerCase())
    );
    set({ filtered }, false, "updateFilteredPressRelease");
  },

});

export const usePressReleaseStore = create<State>()(
  devtools(withHttpRequest(storeApi), {
    name: "PressReleases Store",
  })
);
