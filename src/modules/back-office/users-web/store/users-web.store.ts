...
import { UserWebService as ApiService } from "../services/users-web.service";

type Payload = PayloadUserWeb;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getUserWebSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredUserWeb");
  },

});

export const useUserWebStore = create<State>()(
