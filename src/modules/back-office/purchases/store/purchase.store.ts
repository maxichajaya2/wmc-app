...
import { ActionsTypes, Purchase as Entity, PayloadPurchase as Payload } from "@/models";
import { PurchaseService as ApiService } from "../services/purchase.service";
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getPurchaseSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    const filtered = data.filter(
      (item) =>
        ...
        item.data.transactionDocumentId?.toLowerCase().includes(filterTerm.toLowerCase())
    );
    set({ filtered }, false, "updateFilteredPurchase");
  },
});

export const usePurchasesStore = create<State>()(
