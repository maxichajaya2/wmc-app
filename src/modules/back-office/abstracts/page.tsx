...
function AbstractsManagementPage() {
  const loading = useAbstractStore(state => state.loading);
  const isOpenDialog = useAbstractStore(state => state.isOpenDialog);
  const isOpenDialogUsers = useAbstractStore(state => state.isOpenDialogUsers);
  const filtered = useAbstractStore(state => state.filtered);
  const filterTerm = useAbstractStore(state => state.filterTerm);

  const findAll = useAbstractStore(state => state.findAll);
  const findCategories = useCategoryStore(state => state.findAll);
  // const findAllUserWeb = useUserWebStore(state => state.findAll);
  const setFilterTerm = useAbstractStore(state => state.setFilterTerm);
  ...
  return (
    <main className="grid flex-1 items-start p-2 overflow-auto">
      ...
      <div className="overflow-auto">
        <Card x-chunk="dashboard-06-chunk-0">
          <CardHeader>
            ...
            <CardDescription>
              Lista de Absttracts registrados.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {hasReadPermission && (
      ...
      {isOpenDialog && <AbstractDialog />}
      {isOpenDialogUsers && <ReviewersDialog />}
    </main>
  )
}

export default AbstractsManagementPage