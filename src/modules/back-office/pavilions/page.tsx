...
import { usePavilionStore } from "./store/pavilion.store"
import CustomerFilters from "./components/Filter/FilterDialog"
import PavilionDialog from "./components/Dialog/Dialog"
import { useCheckPermission } from "@/utils"
import { ActionRoles, ModulesRoles } from "@/constants"

function PavilionsManagementPage() {
  const loading = usePavilionStore(state => state.loading);
  const isOpenDialog = usePavilionStore(state => state.isOpenDialog);
  const filtered = usePavilionStore(state => state.filtered);
  const filterTerm = usePavilionStore(state => state.filterTerm);

  const findAll = usePavilionStore(state => state.findAll);
  const setFilterTerm = usePavilionStore(state => state.setFilterTerm);
  const openActionModal = usePavilionStore(state => state.openActionModal);
    ...
    <main className="grid flex-1 items-start p-2 overflow-auto">
      ...
      <div className="overflow-auto">
        <Card x-chunk="dashboard-06-chunk-0">
          <CardHeader>
            <CardTitle>Pabellones</CardTitle>
            <CardDescription>
              Lista de pabellones registrados.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {hasReadPermission && (
      ...
      {isOpenDialog && <PavilionDialog />}
    </main>
  )
}

export default PavilionsManagementPage