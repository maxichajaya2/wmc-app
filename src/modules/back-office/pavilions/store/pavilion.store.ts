...
import { ActionsTypes, Pavilion as Entity, PayloadPavilion } from "@/models";
import { PavilionService as ApiService } from "../services/pavilion.service";
import { useUsersStore } from "@/modules/back-office/users/store/users.store";

type Payload = PayloadPavilion;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getPavilionSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredPavilion");
  },

});

export const usePavilionStore = create<State>()(
  devtools(withHttpRequest(storeApi), {
    name: "Pavilions Store",
  })
);
