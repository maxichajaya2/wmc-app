...
import { BlocksService as ApiService } from "../services/block.service";
import { useUsersStore } from "@/modules/back-office/users/store/users.store";

type Payload = PayloadBlock;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll({}),
      (data) => {
        set({ data, filtered: data }, false, "getBlockSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredBlock");
  },

});

export const useBlockStore = create<State>()(
