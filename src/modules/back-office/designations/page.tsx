...
import { useCategoryStore } from "./store/designations.store"
import CustomerFilters from "./components/Filter/FilterDialog"
import CategoryDialog from "./components/Dialog/Dialog"
import { useCheckPermission } from "@/utils"
import { ActionRoles, ModulesRoles } from "@/constants"

function CategorysManagementPage() {
  const loading = useCategoryStore(state => state.loading);
  const isOpenDialog = useCategoryStore(state => state.isOpenDialog);
  const filtered = useCategoryStore(state => state.filtered);
  const filterTerm = useCategoryStore(state => state.filterTerm);

  const findAll = useCategoryStore(state => state.findAll);
  const setFilterTerm = useCategoryStore(state => state.setFilterTerm);
  const openActionModal = useCategoryStore(state => state.openActionModal);


  const hasReadPermission = useCheckPermission(
    ModulesRoles.CATEGORIES,
    ActionRoles.READ
  );
    ...
    <main className="grid flex-1 items-start p-2 overflow-auto">
      <div className="flex items-center py-2 gap-2">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {useCheckPermission(ModulesRoles.CATEGORIES, ActionRoles.CREATE) && (
      ...
      <div className="overflow-auto">
        <Card x-chunk="dashboard-06-chunk-0">
          <CardHeader>
            ...
            <CardDescription>
              Lista de tipos de conferencia registrados.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {hasReadPermission && (
      ...
      {isOpenDialog && <CategoryDialog />}
    </main>
  )
}

export default CategorysManagementPage