...
import { ActionsTypes, Fee as Entity, PayloadFee } from "@/models";
import { FeeService as ApiService } from "../services/fee.service";

type Payload = PayloadFee;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getFeeSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredFee");
  },

});

export const useFeeStore = create<State>()(
