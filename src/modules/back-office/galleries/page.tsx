...
import CustomerFilters from "./components/Filter/GalleryFilter"
import GalleryActionDialog from "./components/Dialog/GalleryDialog"
...
function RolesPage() {
  ...
  const filtered = useGalleryStore(state => state.filtered);
  const filterTerm = useGalleryStore(state => state.filterTerm);

  const findAll = useGalleryStore(state => state.findAll);
  const setFilterTerm = useGalleryStore(state => state.setFilterTerm);
  ...
  return (
    <main className="grid flex-1 items-start p-2 overflow-auto">
      ...
      {isOpenDialog && <GalleryActionDialog />}
    </main>
  )
}
