...
import { ActionsTypes, Gallery as Entity, PayloadGallery } from "@/models";
import { GalleryService as ApiService } from "../services/gallery.service";

type Payload = PayloadGallery;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll({}),
      (data) => {
        set({ data, filtered: data }, false, "getGallerySuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredGallery");
  },
});

export const useGalleryStore = create<State>()(
