...
import { ActionsTypes, ConferenceType as Entity, PayloadConferenceType } from "@/models";
import { ConferenceTypeService as ApiService } from "../services/conference-type.service";
import { useUsersStore } from "@/modules/back-office/users/store/users.store";

type Payload = PayloadConferenceType;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getConferenceTypeSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredConferenceType");
  },

});

export const useConferenceTypeStore = create<State>()(
  devtools(withHttpRequest(storeApi), {
    name: "ConferenceTypes Store",
  })
);
