...
import ConferenceTypeDialog from "./components/Dialog/ConferenceTypeDialog"
import { useCheckPermission } from "@/utils"
import { ActionRoles, ModulesRoles } from "@/constants"

function ConferenceTypesManagementPage() {
  ...
  const filtered = useConferenceTypeStore(state => state.filtered);
  const filterTerm = useConferenceTypeStore(state => state.filterTerm);

  const findAll = useConferenceTypeStore(state => state.findAll);
  const setFilterTerm = useConferenceTypeStore(state => state.setFilterTerm);
  const openActionModal = useConferenceTypeStore(state => state.openActionModal);
    ...
    <main className="grid flex-1 items-start p-2 overflow-auto">
      ...
      <div className="overflow-auto">
        <Card x-chunk="dashboard-06-chunk-0">
          <CardHeader>
            <CardTitle>Tipos de conferencia</CardTitle>
            <CardDescription>
      ...
      {isOpenDialog && <ConferenceTypeDialog />}
    </main>
  )
}

export default ConferenceTypesManagementPage