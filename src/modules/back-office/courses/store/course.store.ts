...
import { ActionsTypes, Course as Entity, PayloadCourse } from "@/models";
import { CourseService as ApiService } from "../services/course.service";
import { useUsersStore } from "@/modules/back-office/users/store/users.store";

type Payload = PayloadCourse;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getCourseSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredCourse");
  },
