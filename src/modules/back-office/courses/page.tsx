...
import { useConferenceTypeStore } from "../conference-type/store/conference-type.store"

function CoursesManagementPage() {
  const loading = useCourseStore(state => state.loading);
  const isOpenDialog = useCourseStore(state => state.isOpenDialog);
  const filtered = useCourseStore(state => state.filtered);
  const filterTerm = useCourseStore(state => state.filterTerm);

  const findAll = useCourseStore(state => state.findAll);
  const findAllConferenceTypes = useConferenceTypeStore(state => state.findAll);
  const setFilterTerm = useCourseStore(state => state.setFilterTerm);
