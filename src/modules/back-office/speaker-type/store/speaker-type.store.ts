...
import { ActionsTypes, SpeakerType as Entity, PayloadSpeakerType } from "@/models";
import { SpeakerTypeService as ApiService } from "../services/speaker-type.service";
import { useUsersStore } from "@/modules/back-office/users/store/users.store";

type Payload = PayloadSpeakerType;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getSpeakerTypeSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredSpeakerType");
  },

});

export const useSpeakerTypeStore = create<State>()(
  devtools(withHttpRequest(storeApi), {
    name: "SpeakerTypes Store",
  })
);
