...
import { PageWebService as ApiService } from "../services/page.service";
import { useUsersStore } from "@/modules/back-office/users/store/users.store";

type Payload = PayloadPageWeb;
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll({}),
      (data) => {
        set({ data, filtered: data }, false, "getPageSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredPage");
  },
