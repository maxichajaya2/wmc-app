...
import {
  ActionsTypes,
  Supplier as Entity,
  PayloadSupplier as Payload,
} from "@/models";
import { SuppliersService as ApiService } from "../services/suppliers.service";
...
export const storeApi: StateCreator<State, [["zustand/devtools", never]]> = (
  ...
  async findAll() {
    handleRequestStore(
      ...
      () => ApiService.findAll(),
      (data) => {
        set({ data, filtered: data }, false, "getSupplierSuccess");
        get().clearFilters();
      },
  ...
  updateFiltered() {
    ...
    set({ filtered }, false, "updateFilteredSupplier");
  },
});

export const useSuppliersStore = create<State>()(
