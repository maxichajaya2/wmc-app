...
function PapersDialog() {
    ...
    useEffect(() => {
        (
            async () => {
                if (selected) {
                    console.log(selected.eventDate ? DateClass.DateToFormat(selected.eventDate, DateClass.FORMAT_INPUT_SHORT) : undefined)
                    form.reset({
                        ...selected,
                        file: selected.file || '',
    ...
    return (
        <Dialog open={isOpenDialog} onOpenChange={(open) => {
            ...
            <DialogContent
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="w-full grid grid-cols-1 gap-0 md:gap-8 space-y-6 min-h-max">
